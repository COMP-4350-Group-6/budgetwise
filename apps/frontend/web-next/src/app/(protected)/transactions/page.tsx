...
export default function TransactionsPage() {
  ...
  // ===== Load Data (memoized to avoid recreating on each render) =====
  const loadTransactions = useCallback(async () => {
    try {
      setLoading(true);
      const txList = await transactionService.listTransactions({ days: 90, limit: 500 });
  ...
  // Initial data load - services are stable, so this only runs once
  useEffect(() => {
    let cancelled = false;

    const load = async () => {
      try {
        const [cats, bs, txList] = await Promise.all([
          categoryService.listCategories(true),
          budgetService.listBudgets(true),
          transactionService.listTransactions({ days: 90, limit: 500 }),
        ]);
  ...
  }, [categoryService, budgetService, transactionService]);
  ...
  const handleEditTransaction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedTx = await transactionService.updateTransaction(editTx.id, {
    ...
    <div className={styles.pageContainer}>
      ...
      <UploadInvoiceModal
        ...
        onParseInvoice={async () => {
          ...
          const parsed = await transactionService.parseInvoice(uploadedImage);
          if (parsed) {
