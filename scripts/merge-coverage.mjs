...
function mergeCoverage() {
  console.log('🔍 Searching for coverage reports...\n');
  ...
  for (const source of COVERAGE_SOURCES) {
    ...
    if (!existsSync(coveragePath)) {
      console.log(`⏭️  Skipping ${source} (no coverage directory)`);
      continue;
    }
    ...
    if (lcovFiles.length === 0) {
      console.log(`⏭️  Skipping ${source} (no lcov.info found)`);
      continue;
    }
    ...
    for (const lcovFile of lcovFiles) {
      try {
        ...
        totalFiles++;
        console.log(`✅ Processed: ${relative(ROOT_DIR, lcovFile)}`);
      } catch (error) {
        console.error(`❌ Error reading ${lcovFile}:`, error.message);
      }
    }
  }
  
  if (totalFiles === 0) {
    console.log('\n⚠️  No coverage reports found. Run tests with coverage first:');
